const { getFromAPIResponse, getSizeFromAPIResponse } = require('../../utils/Json');
const { PUBLIC_API_URL } = require('../constants/Endpoints');
const { WAIT_FOR_SHIPMENT_STATUS, WAIT_FOR_SHIPMENT_PROPERTY } = require('../constants/Waiting');
const PublicAPIVersion = require('../enums/PublicAPIVersion');
const { ShipmentStatus } = require('../enums/ShipmentStatus');
const { getHeaders, sendGetRequest, sendPostRequest } = require('../../utils/Fetcher');
 ...
 */
export async function waitForShipmentStatusWithTimeout(publicApiToken, shipmentId, status, timeout) {
    
    let response = await retrieveShipment(publicApiToken, shipmentId);
    let currentStatus = getFromAPIResponse(response, "data.status");
    let time = 0;

    while (currentStatus !== status.getPPStatus()) {
        // await wait(MEDIUM_WAIT);
        time += MEDIUM_WAIT;
        response = await retrieveShipment(publicApiToken, shipmentId);
        currentStatus = getFromAPIResponse(response, "data.status");
        
        if (time > timeout) {
            throw new Error(`Shipment status is not updated on DB after ${timeout / 1000} seconds!`);
        }
    }
}
 ...
 * @param {ShipmentStatus} status - Expected shipment status to wait for
 */
export async function waitForShipmentStatus(publicApiToken, shipmentId, status) {
    await waitForShipmentStatusWithTimeout(publicApiToken, shipmentId, status, WAIT_FOR_SHIPMENT_STATUS);
}
 ...
 */
export async function updateShipmentWithVersion(publicApiToken, shipmentUuid, shipmentId, body, version) {
 ...
 */
export async function updateShipment(publicApiToken, shipmentUuid, shipmentId, body) {
    return updateShipmentWithVersion(publicApiToken, shipmentUuid, shipmentId, body, PublicAPIVersion.V_5_0_0);
}
...
export async function updateShipmentById(publicApiToken, shipmentId, body) {
    return updateShipmentWithVersion(publicApiToken, "<ignore>", shipmentId, body, PublicAPIVersion.V_5_0_0);
}
 ...
 */
export async function retrieveShipmentWithUuid(publicApiToken, shipmentUuid, shipmentId) {
 ...
 */
export async function retrieveShipment(publicApiToken, shipmentId) {
    return retrieveShipmentWithUuid(publicApiToken, "<ignore>", shipmentId);
}
 ...
 */
export async function isShipmentProperty(publicApiToken, shipmentId, jsonPath, expectedValue) {
 ...
 */
export async function waitForShipmentPropertyWithTimeout(publicApiToken, shipmentId, jsonPath, expectedValue, timeout) {
    
    let time = 0;
    while (!(await isShipmentProperty(publicApiToken, shipmentId, jsonPath, expectedValue))) {
        //  await wait(MEDIUM_WAIT);
        time += MEDIUM_WAIT;
        if (time > timeout) {
            return false;
 ...
 */
export async function waitForShipmentProperty(publicApiToken, shipmentId, jsonPath, expectedValue) {
    return waitForShipmentPropertyWithTimeout(publicApiToken, shipmentId, jsonPath, expectedValue, WAIT_FOR_SHIPMENT_PROPERTY);
}
 ...
 */
export async function isShipmentEventUpdated(publicApiToken, shipmentId, eventKey) {
    
    const response = await retrieveShipment(publicApiToken, shipmentId);
    return getSizeFromAPIResponse(response, `data.all_events[?(@.event_key=='${eventKey}')]`) > 0;
}
 ...
 */
export async function waitForShipmentEventUpdatedWithTimeout(publicApiToken, shipmentId, eventKey, timeout) {
    
    let time = 0;
    while (!(await isShipmentEventUpdated(publicApiToken, shipmentId, eventKey))) {
        //  await wait(MEDIUM_WAIT);
        time += MEDIUM_WAIT;
        if (time > timeout) {
            return false;
 ...
 */
export async function waitForShipmentEventUpdated(publicApiToken, shipmentId, eventKey) {
    return waitForShipmentEventUpdatedWithTimeout(publicApiToken, shipmentId, eventKey, WAIT_FOR_SHIPMENT_PROPERTY);
}
 ...
 * @returns {Promise<boolean>} True if property matches expected value
 */
export async function isListShipmentProperty(publicApiToken, queryString, jsonPath, expectedValue) {
    try {
 ...
 * @returns {Promise<boolean>} True if property matches within timeout period
 */
export async function waitForListShipmentProperty(publicApiToken, queryString, jsonPath, expectedValue, timeout) {
    
    let time = 0;
    while (!(await isListShipmentProperty(publicApiToken, queryString, jsonPath, expectedValue))) {
        //  await wait(MEDIUM_WAIT);
        time += MEDIUM_WAIT;
        if (time > timeout) {
            return false;
...
export async function trackShipment(publicApiToken, body) {
    return trackShipmentWithVersion(publicApiToken, body, PublicAPIVersion.V_5_2_0);
}
 ...
 * @returns {Promise<Response>} Response containing shipment details
 */
export async function getShipment(publicApiToken, publicAPIVersion, queryString) {
